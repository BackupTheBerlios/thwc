 ...
 else
 {
	 ...
	 else
	 {
		 ...
		 if( $config['polls_enable'] == 1 )
		 {
			 ...
			 if( P_POLLNEW == 1 )
			 {
			     $data['pollopt1'] .= '&nbsp;||&nbsp;<a href="newpoll.php?boardid='.$board['board_id'].'" class="bg"><b>Neue Umfrage erstellen</b></a>';
		     ...
		     while( $thread = db_result( $r_thread ) )
			 {
				 ...
				 else
				 {
					 ...
					 if( $thread['link'] == $board['board_id'] && P_OMOVE == 1 )
					     $topic .= '&nbsp;[smallfont][<a href="threadopt.php?action=dellink&threadid='.$thread['thread_id'].'">Link entfernen</a>][smallfontend]';
					 // replies
